(($, window)=>{ //화살표 함수
   ...
   const SignUp = {
        회원 : {
         가입회원:[], //array
         아이디:'', //string
         아이디중복확인:false,  //bool(참거짓)
         비밀번호:'', //string
         비밀번호확인:'', //string
         이름:'', //string
         이메일:'', //string
         이메일중복확인:false,//bool(참거짓)
         휴대폰:'', //string
         휴대폰인증확인:false,//bool(참거짓)
         휴대폰인증번호:'', //string
         setId: 0, //number
         주소1:'', //string
         주소2:'', //string
            ...
            $.ajax({
               ...
               success: function(res){
                  console.log(JSON.parse(res));
                   that.회원.가입회원 = JSON.parse(res);  // 데이터베이스 저장
                  },
               error: function(err){
         ...
         $('.id-ok-btn').on({
            click: function(){
               ...
               if($('#id').val()===''){ // 공백이면
                  ...
                  // 특수문자는 삭제
                  if( regExp2.test($('#id').val()) === false  ||  regExp3.test($('#id').val()) === false || regExp4.test($('#id').val()) === true ){
                     $('.member-modal').fadeIn(300);
         ...
         $('.member-modal-close-btn').on({
            click: function(){
               ...
               if(that.회원.가입완료 === true){
                  location.href = path;  //php 에서 전달해준 루트 위치의 변수
               }
            }
         })
         ...
         $('.gender-btn').each(function(idx,item){
            ...
            $(this).on({ // 개체 태그 요소 $('.gender-btn') 
               change: function(){
                  ...
                  // console.log( this );           // 개체 태그 요소 <input type="radio" id='female'   class='gender-btn' name='gender' value='여자'>여자</label>             
                  // console.log( that );           // 객체(Object) SignUp              
                  // console.log( that.회원 );          // 객체(Object) SignUp.gender 속성값 출력                
               }
            });
         });
      },
...
/*       serviceFn(){ //이용약관동의
         ...
         function checkedFn(){
            ...
            $('.check-service').each(function(idx, item){
               $(this).on({
                  change: function(){
                     checkStateFn();
                     checkedFn(); // 체크된 갯수를 카운트 7개이면 전체 체크박스체크
                  }
               });
            });
      ...
      idCheckOk(){
         ...
         // 2. 아이디 중복 검사
               ...
               // 데이터 베이스 조회하면 더이상 로컬스토레이지는 사용 안 함
               //------------------------------------------------------
               // 로컬스토레이지 회원 데이터 가져오기 (테스트 점검용, 데이터베이스 사용하기 전까지)
               // for(let i=0; i<localStorage.length; i++){
               //    imsi = [...imsi, JSON.parse(localStorage.getItem(localStorage.key(i)))];  // 키 값을
               // }

               // 아이디만 추출해서 중복검사 준비
               console.log('입력상자아이디'+$('#id').val());
               console.log(imsi);
      ...
      emailCheckOk(){
         ...
         let imsi = [];
            ...
            if($('#email').val()===''){
               ...
               $('.member-modal').fadeIn(300);
               $('.modal-message').html(`사용가능한 이메일 입니다.`);
         ...
         $('.submit-btn').on({
            click: function(e){
               ...
               // 유효성 검사 : 아이디 ~ 이용약관까지 필수요소(반드시입력)와 선택요소
                  ...
                  that.회원.아이디중복확인 =  that.idCheckOk(),  //로컬스트레이지 저장 후  검증  true or false
                  that.회원.휴대폰인증확인 = that.emailCheckOk(),
                  that.회원.이메일중복확인 =  that.hpCheckOk(), //로컬스트레이지 저장 후  검증 true or false
               ...
               let 마켓컬리회원 = {  //테이블이름 market_kurly_member (필드1, 필드2, FIELD(item))
                  ...
                  휴대폰: that.회원.휴대폰.replace(regExpHp, '$1-$2-$3'), // 13 hp 01063294174 => 치환(replace)  010-6329-4174
                  ...
                  가입일자: `${new Date().getFullYear()}-${new Date().getMonth()}-${new Date().getDate()}` // 날짜 gaib

               }



               // localStorage.setItem(마켓컬리회원.아이디, JSON.stringify(회원) );
